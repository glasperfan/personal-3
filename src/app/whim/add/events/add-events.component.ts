...
import { IAddEventArguments, IError, IEvent, WindowView, WhimErrorCode } from '../../models';
...
export class AddCalendarEventsComponent implements OnInit {
  @Input() eventInput: IAddEventArguments;
  ...
  ngOnInit() {
    this.eventInput = this.eventInput || this.defaultInput();
  }
  ...
  private addCalendarEvent(): void {
    // quick validation
    if (!this.eventInput.title) {
      this.processMessage = 'An event title is required';
    } else if (!this.eventInput.date) {
      this.processMessage = 'A date is required';
    } else {
      this.eventInput.date = this.parseDate(<any>this.eventInput.date);
      this.calendarService.addEvents([this.eventInput])
        .then((addedEvents: IEvent[]) => {
          this.processMessage = `${addedEvents[0].title} on ${this.formatDate(addedEvents[0].date)}, got it!`;
          this.eventInput = this.defaultInput();
        })
        .catch((err: IError) => {
